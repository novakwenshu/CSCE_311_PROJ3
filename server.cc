...
int server(int argc, char *argv[]) {
  ...
  while (true) {
    // Unlinks
    sem_unlink("/namesem");
    sem_t *sem = sem_open("/namesem", O_CREAT, S_IRGRP |
                  S_IRUSR | S_IWGRP | S_IWUSR, 0);
    ...
    // MAPS THE MEMORY
    struct shmbuf *store = static_cast<shmbuf*>(mmap(nullptr,
    ...
    char *path = reinterpret_cast<char*>(malloc(sizeof
         (reinterpret_cast<char*>(strlen(store->buffer)+1))));
    FILE* file;
    path = store->buffer;
    ...
    if (file != NULL) {
      ...
      while ((getline(&line, &len, file)) != -1) {
        if ((count != 0 && count%4 == 0)) {
          // Once 4 lines have been put into shared memory
          // tell client that it can read it
          if (sem_post(&store->sem1) == -1) {
